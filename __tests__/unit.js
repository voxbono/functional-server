...
// const result = parseRequestBody ($.Any) (S.Just ('application/x-www-form-urlencoded')) ('%7B%09id%3A%201%2C%09name%3A%20%22Jonas%22%2C%20email%3A%20%22test%40test.com%22%7D=');
// expect (result).toEqual (S.Just ({ 'a': '%3/æøå' }));
// });

test ('Request body application/json', () => {
  const result = parseRequestBody ($.Any) (S.Just ('application/json')) ('{"a":"%3/æøå"}');
  expect (result).toEqual (S.Just ({ 'a': '%3/æøå' }));
});

test ('Request body no content-type', () => {
  const result = parseRequestBody ($.Any) (S.Nothing) ('{"a":"%3/æøå"}');
  expect (result).toEqual (S.Nothing);
});

test ('Request body wrong content-type', () => {
  const result = parseRequestBody ($.Any) (S.Just ('foo')) ('{"a":"%3/æøå"}');
  expect (result).toEqual (S.Nothing);
});
